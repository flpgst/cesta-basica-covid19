...
const listar = (recurso, params = {}) => {
    return servidor.get(`${URL_BASE}/${recurso}`, { params: formatParams(params), headers: getHeaders() })
...
const carregar = (recurso, id) => {
    const url = recurso.includes(':id') 
        ...
        : `${URL_BASE}/${recurso}/${id}`
    return servidor.get(url, { headers: getHeaders() })
        .then(response => Promise.resolve(response.data))
        .catch(handleError)
}
...
const atualizar = (recurso, payload) => {
    return servidor.put(`${URL_BASE}/${recurso}/${payload.id}`, payload, { headers: getHeaders() })
        .then(response => Promise.resolve(response.data))
        .catch(handleError)
}
